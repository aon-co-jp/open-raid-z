...
- **RGit**(Gitea/GitBucket相当、自己ホスト型Git forge。OTPログイン・
  アクセス制御・容量ベースの自動判定まで実装済み、WASM UIも着手済み):
  https://github.com/aon-co-jp/RGit
- **RJSON**(`rust-json`クレート、寛容/厳密JSONパース+依存ゼロの`light`
  モジュール、RGitのWASMフロントエンドが利用): https://github.com/aon-co-jp/RJSON
...
- **バックグラウンド並行開発エージェントの停滞自動検知・自動再開
  ...
  2. エージェントがユーザー操作によって既に停止(`stopped by the
     user`)している場合は、`SendMessage`が失敗しその旨が返る——
     この場合は絶対に新しいエージェントを起動し直さない(ユーザーが
     明示的に再度依頼するまで、その作業は「キャンセル済み」として
     扱う)。未コミットの変更はディスク上に残るため作業自体は失われて
     いないことを依頼者に伝えてよい。
