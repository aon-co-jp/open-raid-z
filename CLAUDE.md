...
- **Androidスマホ・タブレット対応の最優先化(2026-07-31追記、ユーザー
  ...
  3. 各リポジトリのCLAUDE.md HANDOFFに「Android対応: 現状(NDK
     ...
     追跡できるようにする。**2026-08-01時点の進捗**: `aruaru-db`
     (リモート監視クライアント)・`open-redmine`(疎通確認+ブラウザ
     起動クライアント)は`android/`ディレクトリ新設・`gradle
     assembleDebug`成功まで完了(いずれも実機/エミュレータでの実疎通
     確認は未実施)。`rs-link-fusion`・`open-easy-web`は未着手のまま
     残っている。
