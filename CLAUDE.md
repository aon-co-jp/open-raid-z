...
- **RS-AI-DevOps**(エコシステム全体マップ自動生成+AIエージェント向け
  コンテキストファイル(CLAUDE.md/.cursorrules/AGENTS.md)生成+複数Git/
  課題管理サービス(GitHub実装済み・RS-Chiketto実装済み・Bitbucket/
  Gitbucket/GitLab/標準Redmineは未実装)連携。`runo-scanner`+`RS-Sync`の
  資産を土台に製品化、`https://runo.tokyo/RS-AI-DevOps`で稼働中):
  https://github.com/aon-co-jp/RS-AI-DevOps
