...
は、この open-runo(cosmo)コアを中心に据えつつ、Rustの Poem 互換
フレームワーク(自前実装、パッケージ非依存)との「スムーズな連携」——
ルーティング/ハンドラのAPI形状に互換性を保った統合——を行うことが
本リポジトリ固有の役割**。新機能・改善タスクを検討する際は、この
4点(Tauri互換UI・REST API不要・契約不要・自作AI高速化)と、Poem互換
フレームワークとの連携のスムーズさ、の両軸で完成度・利便性・使いやすさ・
実用性を継続的に高めることを目標とする。
