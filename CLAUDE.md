...
- **`open-cuda`**(https://github.com/aon-co-jp/open-cuda): クロスベンダー
  ...
  実装済み。LLM推論に不可欠なAttention/GEMM(`opencuda-blas`クレート)は
  現状「Phase 3」として明示的に未実装(スタブ)。
...
**現状の実装状況(2026-07-18時点)**: `aruaru-llm`にのみ
`src/tenants.rs`(`TenantRegistry`、`POST /admin/tenants`・
`GET /admin/tenants`・`DELETE /admin/tenants/:host`)を実装済み。
`open-cuda`・`RPoem`・`RCosmo`・`open-raid-z`・`aruaru-db`への同パターン
展開、および`open-easy-web`側からこれら全サービスの管理APIを呼び出す
統合(`appserver_registration.rs`の拡張)は**未着手**——次回以降の
実装対象として、各リポジトリのCLAUDE.mdに引き継ぎ事項として記録する。
...
- **aruaru-llm**(契約不要の独自AIチャットコマース応答サービス、`open-cuda`とSET構成): https://github.com/aon-co-jp/aruaru-llm
