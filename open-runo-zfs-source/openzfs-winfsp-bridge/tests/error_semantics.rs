...
//! いなかった)。
//!
//! [`DatasetNotFound`]: openzfs_winfsp_bridge::BridgeError::DatasetNotFound
//! [`SnapshotNotFound`]: openzfs_winfsp_bridge::BridgeError::SnapshotNotFound
//! [`AlreadyExists`]: openzfs_winfsp_bridge::BridgeError::AlreadyExists
//! [`CapacityExceeded`]: openzfs_winfsp_bridge::BridgeError::CapacityExceeded
//! [`InvalidConfig`]: openzfs_winfsp_bridge::BridgeError::InvalidConfig
//! [`Unrecoverable`]: openzfs_winfsp_bridge::BridgeError::Unrecoverable

use openzfs_winfsp_bridge::block_device::{FaultInjectableDevice, FileBackedDevice};
...
use openzfs_winfsp_bridge::BridgeError;
...
fn scratch_dir(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("openruno_error_semantics_it_{name}_{}", std::process::id()));
