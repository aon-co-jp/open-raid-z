...
function renderStaticText(): void {
  ...
  const warning = document.getElementById("no-disks-warning");
...
async function loadHardware(): Promise<void> {
  ...
  const accelEl = document.getElementById("accelerator-info")!;
  ...
  const warningEl = document.getElementById("no-disks-warning")!;
  ...
  for (const item of advice) {
    const li = document.createElement("li");
    li.className = `advice-${item.severity.toLowerCase()}`;
...
function toggleMirrorWidthVisibility(): void {
  const level = (document.getElementById("raid-level") as HTMLSelectElement).value;
  const field = document.getElementById("mirror-width-field")!;
...
async function submitRaidForm(e: SubmitEvent): Promise<void> {
  e.preventDefault();
  const level = (document.getElementById("raid-level") as HTMLSelectElement).value;
  const diskCount = Number((document.getElementById("disk-count") as HTMLInputElement).value);
  ...
  try {
    if (level === "Raid10") {
      const mirrorWidth = Number((document.getElementById("mirror-width") as HTMLInputElement).value);
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("refresh-button")?.addEventListener("click", () => void loadHardware());
  document.getElementById("raid-level")?.addEventListener("change", toggleMirrorWidthVisibility);
  document.getElementById("raid-form")?.addEventListener("submit", (e) => void submitRaidForm(e));
