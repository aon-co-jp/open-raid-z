...
| ext2/ext4 | Linux上でOS標準の`mount`、または`orzctl foreign --format ext4`(2026-07-20追加、本ツール内蔵) | OS任せ、または`orzctl foreign`(**読み取り専用**。`ls`/`cat`/`mount`対応、`put`等の書き込みは非対応) |
...
- **NTFS/他社製RAID**: OS標準の方法でマウントするだけでよい
  (`open-raid-z`は関与しない)。
- **ext2/ext4**: OS標準の方法(Linux上での`mount`)でもよいが、Windows上
  からも`orzctl foreign --format ext4 mount /dev/sdX1 <マウント先>`で
  直接読み取れる(2026-07-20追加、`foreign_fs` feature、読み取り専用)。
