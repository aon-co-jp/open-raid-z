// RAID-Z3 (三重パリティ P/Q/R) 用 Reed-Solomon Compute Shader — NPU専用ディスパッチ経路。
//
// 【現状】アルゴリズムは shaders/raidz3_parity.hlsl (GPU用) と完全に同一
// (経緯は raidnpu_parity.hlsl の先頭コメント参照)。
//
// GF(2^8)(既約多項式 x^8+x^4+x^3+x^2+1 = 0x11d、生成元2)上で
// P = XOR(D_i)、Q = XOR(D_i * 2^i)、R = XOR(D_i * 4^i)(4^i = 2^(2i))を計算する。
// 対応するCPU参照実装・テストは ../src/raidz23_parity.rs::compute_pqr を参照。
