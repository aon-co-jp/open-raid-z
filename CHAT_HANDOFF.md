...
| # | コンポーネント | 役割 | 技術スタック |
|---|---|---|---|
| ① | open_zfs_winfsp_bridge | WinFsp経由でZFSのI/Oをフックし、NTFS ACLセマンティクスをエミュレーション | Rust + WinFsp(winfsp-rs) + windows-rs |
...
- `open_zfs_winfsp_bridge/Cargo.toml`の`[build-dependencies]`で`winfsp`が
...
- `zfs_accel_hlsl/build.rs`が、featureに関わらず常に`dxc`
  ...
  (`zfs_accel_hlsl`は`open_zfs_winfsp_bridge`の非optional依存だったため、
...
(`build.rs`)をこのfeature配下に隔離。`open_zfs_winfsp_bridge`側にも
...
(`open_zfs_winfsp_bridge`側)。`zfs_accel_hlsl`単体は変更無し(20テスト
引き続きパス)。
...
(`open_zfs_winfsp_bridge`側。内訳は前回70+今回追加5)。`zfs_accel_hlsl`は
...
| `openzfs-winfsp-bridge`(crate名`openzfs_winfsp_bridge`) | `open_zfs_winfsp_bridge` |
...
エンド側の名称も含めて全て置換した。置換後、`open_zfs_winfsp_bridge`
...
| 対象 | 検証方法 | 結果 |
|---|---|---|
| `open_zfs_winfsp_bridge` | `cargo test --no-default-features` | 82テスト全パス |
...
3. CI(GitHub Actions)追加。Linux runnerでは`open_zfs_winfsp_bridge`/
...
要望があり、実装した(`open_zfs_winfsp_bridge/src/migrate.rs`、新規)。
