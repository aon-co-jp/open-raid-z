...
| コンポーネント | 役割・現状 |
|---|---|
| `open_raid_z_core` | 中核ライブラリ。RAIDレベル(`Raid0`/`Raid1`/`Raid5`/`Raid6`≡`Z2`/`Z3`。`vdev.rs`の`RaidLevel` enum)、sha2チェックサム、Copy-on-Write、スナップショット/クローン、ACLエミュレーション、FAT32/exFAT相互運用(`foreign_fs` feature、書き込み対応)、実マウント(Windows=WinFsp、Linux/macOS/Android=FUSE)、`orzctl`CLIバイナリを含む |
