...
- **Ponte di lettura/scrittura per formati esterni (`foreign_fs`)**: distinto dal formato di pool proprio di open-raid-z, consente di leggere e scrivere volumi FAT32/FAT16 ed exFAT già esistenti creati da altri sistemi operativi (chiavette USB, microSD, schede CF, ecc.) — ora anche la scrittura exFAT funziona, tramite il crate `hadris-fat`. Operabile tramite `orzctl foreign` (`ls`/`cat`/`put`)
...
## Migrazione da una configurazione esistente

Per i passaggi di migrazione dei dati da una configurazione ZFS/NTFS/ext4/altro RAID esistente a `open-raid-z`, vedere [MIGRATION.md](../MIGRATION.md).
