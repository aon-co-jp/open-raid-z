...
- **GPU/NPU hardware acceleration**: RAID-Z1/Z2/Z3 parity generation is dispatched via DirectX 12 Compute + DirectML (automatically falls back to CPU when no hardware is present). Also implements a scheme that converts GF(2^8) coefficient multiplication into a GF(2) bit matrix and reduces it to a single DirectML GEMM dispatch (`zfs_accel_hlsl::dml_gemm`), verified correct on real GPU hardware (not yet verified on real NPU hardware). The same mechanism is wired into the reconstruction computation that scrub/resilver runs when corruption is detected (i.e. parity checking). Dedicated NPU shader paths (`raidnpu_*.hlsl`) are also provided, in preparation for future verification/optimization on real NPU hardware
- **Vulkan Compute acceleration for non-Windows platforms**: DirectX/DirectML is a Windows-only API, so a Vulkan Compute implementation via the `ash` crate was added for Linux/Mac/Android (`zfs_accel_hlsl::vulkan_compute`, the `vulkan` feature). Beyond RAID-Z1 XOR parity, RAID-Z2/Z3 (P/Q/R parity generation) are also Vulkan-accelerated now, via `raidz2_parity.comp`/`raidz3_parity.comp` (GLSL). All paths were verified to match the CPU reference implementation on real GPU hardware (NVIDIA GeForce GT 730, Vulkan 1.2)
- **NPU/GPU/CPU performance benchmark**: actually measures the real-world throughput (MB/s) of each detected accelerator (`zfs_accel_hlsl::benchmark`, runnable from the installer's "Compatibility" panel), rather than assuming a detected GPU/NPU is automatically faster than the CPU (low-end integrated GPUs sometimes aren't). Also raises the compute queue priority (D3D12 `HIGH`, Vulkan maximum) so this app's GPU/NPU work is scheduled ahead of other processes sharing the same hardware
- **Foreign-format read/write bridge (`foreign_fs`)**: separate from open-raid-z's own pool format, it can read and write existing FAT32/FAT16 and exFAT volumes created by other OSes (USB sticks, microSD, CF cards, etc.) — exFAT write support now works too, via the `hadris-fat` crate. Operable via `orzctl foreign` (`ls`/`cat`/`put`), or actually mounted on Linux/macOS via the `mount` subcommand, exposing a real directory hierarchy (mkdir/rmdir/rename included)
- **Installer "Compatibility Status" panel**: openable/closable via a button, showing the current OS's support status, every detected GPU/NPU (with vendor detection: Intel/AMD/NVIDIA/Qualcomm, supports multiple devices), and detected storage media types (HDD/SSD/NVMe/USB/SD/CF)
...
This project aims to make open-raid-z itself readable and writable on Windows/Mac/Linux/Android/iOS/iPad, and interoperable with existing other-OS formats (NTFS/exFAT/FAT32/ext4/APFS, etc). Current feasibility, priority, and technical constraints — most notably that Apple does not allow third-party block-device RAID on iOS/iPad, so support there will likely be limited to browsing via a File Provider Extension — are recorded in [`MULTIPLATFORM_ROADMAP.md`](open_runo_zfs_source/open_raid_z_core/contrib/systemd/MULTIPLATFORM_ROADMAP.md). GPU/NPU acceleration will progressively adopt each OS's native API where DirectX isn't available (e.g. Metal Performance Shaders on Mac, NNAPI on Android). Interoperability with third-party RAID formats (e.g. Linux mdadm, Windows Storage Spaces) is also being considered as a future scope item.
