...
- **Vulkan Compute acceleration for non-Windows platforms**: DirectX/DirectML is a Windows-only API, so a Vulkan Compute implementation via the `ash` crate was added for Linux/Mac/Android (`zfs_accel_hlsl::vulkan_compute`, the `vulkan` feature). RAID-Z1 XOR parity generation was verified to work correctly on real GPU hardware (NVIDIA GeForce GT 730, Vulkan 1.2)
- **Foreign-format read/write bridge (`foreign_fs`)**: separate from open-raid-z's own pool format, it can read and write existing FAT32/FAT16 volumes created by other OSes (USB sticks, microSD, CF cards, etc.), and read exFAT volumes (exFAT is currently read-only due to an upstream crate limitation). Operable via `orzctl foreign` (`ls`/`cat`/`put`)
...
- **Multilingual support**: the installer (OpenRaidZ Installer) defaults to English with a 10-language UI switcher, changeable after installation too
