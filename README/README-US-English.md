...
- **Foreign-format read/write bridge (`foreign_fs`)**: separate from open-raid-z's own pool format, it can read and write existing FAT32/FAT16 and exFAT volumes created by other OSes (USB sticks, microSD, CF cards, etc.) — exFAT write support now works too, via the `hadris-fat` crate. Operable via `orzctl foreign` (`ls`/`cat`/`put`)
