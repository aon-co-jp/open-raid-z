...
- **Fremdformat-Lese-/Schreibbrücke (`foreign_fs`)**: Getrennt vom eigenen Poolformat von open-raid-z ermöglicht diese Brücke das Lesen und Schreiben bestehender FAT32-/FAT16- und exFAT-Volumes, die von anderen Betriebssystemen erstellt wurden (USB-Sticks, microSD, CF-Karten usw.) — dank des `hadris-fat`-Crates funktioniert nun auch das Schreiben auf exFAT. Bedienbar über `orzctl foreign` (`ls`/`cat`/`put`).
