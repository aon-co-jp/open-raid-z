...
- **Vulkan-Compute-Beschleunigung für Nicht-Windows-Plattformen**: DirectX/DirectML ist eine reine Windows-API, daher wurde für Linux/Mac/Android zusätzlich eine Vulkan-Compute-Implementierung über das `ash`-Crate ergänzt (`zfs_accel_hlsl::vulkan_compute`, Feature `vulkan`). Die RAID-Z1-XOR-Paritätserzeugung wurde auf echter GPU-Hardware (NVIDIA GeForce GT 730, Vulkan 1.2) als korrekt funktionierend verifiziert.
- **Fremdformat-Lese-/Schreibbrücke (`foreign_fs`)**: Getrennt vom eigenen Poolformat von open-raid-z ermöglicht diese Brücke das Lesen und Schreiben bestehender FAT32-/FAT16-Volumes, die von anderen Betriebssystemen erstellt wurden (USB-Sticks, microSD, CF-Karten usw.), sowie das Lesen von exFAT-Volumes (exFAT ist aufgrund einer Einschränkung im verwendeten Upstream-Crate derzeit nur lesbar, nicht beschreibbar). Bedienbar über `orzctl foreign` (`ls`/`cat`/`put`).
- **„Kompatibilitätsstatus"-Panel im Installer**: über eine Schaltfläche ein-/ausblendbar, zeigt es den Unterstützungsstatus des aktuellen Betriebssystems, jede erkannte GPU/NPU (mit Herstellererkennung: Intel/AMD/NVIDIA/Qualcomm, auch bei mehreren erkannten Geräten) sowie die erkannten Speichermedientypen (HDD/SSD/NVMe/USB/SD/CF) an.
...
- **Mehrsprachige Unterstützung**: Der Installer (OpenRaidZ Installer) verwendet standardmäßig Englisch mit einem Sprachumschalter (9 Sprachen: Englisch, Japanisch, Italienisch, Französisch, Deutsch, Russisch, Ukrainisch, Arabisch, Persisch) in der Benutzeroberfläche, der auch nach der Installation geändert werden kann. Die Hybridanzeige (Hauptsprache + eine wählbare zweite Sprache, standardmäßig Japanisch) ist standardmäßig aktiviert und zeigt beide Sprachen nebeneinander an
...
## Roadmap: Multi-OS- und Cross-Format-Interoperabilität

Ziel des Projekts ist es, dass open-raid-z selbst künftig auch unter Windows/Mac/Linux/Android/iOS/iPad gelesen und geschrieben werden kann und mit bestehenden Formaten anderer Betriebssysteme (NTFS/exFAT/FAT32/ext4/APFS usw.) interoperabel wird.

Der aktuelle Stand hinsichtlich Machbarkeit, Priorisierung und technischer Einschränkungen – insbesondere, dass Apple Drittanbietern kein blockbasiertes RAID auf iOS/iPad erlaubt, wodurch sich die Unterstützung dort voraussichtlich auf das Durchsuchen über eine File Provider Extension beschränken wird – ist in [`MULTIPLATFORM_ROADMAP.md`](open_runo_zfs_source/open_raid_z_core/contrib/systemd/MULTIPLATFORM_ROADMAP.md) festgehalten.
