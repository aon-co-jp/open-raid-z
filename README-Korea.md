...
| 컴포넌트 | 역할/현황 |
|---|---|
| `open_raid_z_core` | 핵심 라이브러리: RAID 레벨(`Raid0`/`Raid1`/`Raid5`/`Raid6`≡`Z2`/`Z3`, `vdev.rs`의 `RaidLevel` 열거형), sha2 체크섬, Copy-on-Write, 스냅샷/클론, ACL 에뮬레이션, FAT32/exFAT 상호운용(`foreign_fs`, 읽기+쓰기) 및 읽기 전용 ext2/ext4 접근(동일 feature), 실제 마운트(Windows=WinFsp, Linux/macOS/Android=FUSE), `orzctl` CLI 바이너리 |
